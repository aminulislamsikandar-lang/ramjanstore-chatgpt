...
export async function validateCoupon(code:string,subtotal:number,userId:string,categoryIds:string[]=[]):Promise<CouponResult>{
 const c=await getCouponByCode(code); if(!c) throw new Error("Invalid or inactive coupon");
 const now=Date.now();
 if(c.startsAt && c.startsAt.toMillis()>now) throw new Error("Coupon is not active yet");
 if(c.expiresAt && c.expiresAt.toMillis()<now) throw new Error("Coupon has expired");
 if(!Number.isFinite(subtotal)||subtotal<0) throw new Error("Invalid subtotal");
 if(subtotal<c.minOrderValue) throw new Error(`Minimum order value is ${c.minOrderValue}`);
 if(c.usageLimit!==undefined&&c.usageCount>=c.usageLimit) throw new Error("Coupon usage limit reached");
 if(c.perUserLimit!==undefined){ const used=await getUserCouponUsage(userId,c.id); if(used>=c.perUserLimit) throw new Error("Your coupon usage limit has been reached"); }
 if(c.categoryIds?.length&&!categoryIds.some(x=>c.categoryIds!.includes(x))) throw new Error("Coupon does not apply to these products");
 if(!Number.isFinite(c.value)||c.value<0) throw new Error("Invalid coupon value");
 const discount=c.type==="PERCENTAGE"?Math.min(subtotal* c.value/100,c.maxDiscount??Infinity):Math.min(c.value,subtotal);
 return {coupon:c,discount:Math.max(0,Math.round(discount*100)/100)};
}
